...
from anchors import (
    ...
    anchors_for_shape,
    guess_shapes,
    flexible_anchor_generator
)
from config import parse_anchor_parameters
...
class Generator(Sequence):
    ...
    def __init__(
            ...
            anchor_generation_method="multiple",
            is_square_resize=False,
            config=None
    ):
        ...
        self.config = config
        self.image_size = 512
        self.anchor_generation_method = anchor_generation_method
        self.is_square_resize = is_square_resize
    ...
    def preprocess_group_entry(self, image, annotations):
        ...
        # resize image
        image, image_scale, image_scale = self.resize_image(image, is_square_resize=self.is_square_resize)
    ...
    def compute_inputs(self, image_group):
        ...
        max_shape = tuple(max(image.shape[x] for image in image_group) for x in range(3))  # (300, 300, 3)

        # construct an image batch object
        image_batch = np.zeros((self.batch_size,) + max_shape, dtype=keras.backend.floatx())  # (B, 300, 300, 3)
    ...
    def generate_anchors(self, image_shape):
        ...
        if self.anchor_generation_method == "flexible":
            return flexible_anchor_generator(image_shape=image_shape, shapes_callback=self.compute_shapes)
    ...
    def compute_targets(self, image_group, annotations_group):
        ...
        # get the max image shape
        max_shape = tuple(max(image.shape[x] for image in image_group) for x in range(3))  # (300, 300, 3)
        anchors = self.generate_anchors(max_shape)    # (B, 17451, 4)
    ...
    def compute_input_output(self, group):
        ...
        image_group = self.load_image_group(group)  # load images numpy arrays of batch_size
        annotations_group = self.load_annotations_group(group)  # load annotations for batch_size:
        # annotations = {'boxes': [N, 4], 'labels': [N, 1] }
        ...
        inputs = self.compute_inputs(image_group)  # (B, 300, 300, 3)
